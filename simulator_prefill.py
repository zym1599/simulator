...
def estimate_cross_device_time(
    ...
    #在单批次（single batch）且模型层严格串行执行的情况下，total_time 的正确计算方式应为 加和
    #假设 MLA 和 FFN 可跨设备并行（如 MLA 在 GPU、FFN 在 RRAM），但实际模型层的串行依赖使得这种并行无法实现。
    #即使跨设备部署，也必须等待前一层的 MLA 和 FFN 全部完成后，才能执行下一层。#
