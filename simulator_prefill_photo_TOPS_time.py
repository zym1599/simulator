...
def estimate_cross_device_time(
    ...
    # ---------------------- 总时间 ----------------------
    total_time = mla_time + ffn_time + transfer_time + mem_time   ##单batch的情况下，ffn的时间和mla认为不能同时##########################需讨论
...
if __name__ == "__main__":
    ...
    for tops in range(50, 201):
        ...
        total_time_values.append(result["Total_time(ms)"])

    # 输出部分查看
    print("Tops -> Total_time(ms):")
    for i in range(len(tops_values)):
        print(f"{tops_values[i]}  =>  {total_time_values[i]}")

    # 画折线图
    plt.plot(tops_values, total_time_values)
    plt.xlabel("TOPS (Hardware A)")
    plt.ylabel("Total_time (ms)")
    plt.title("Prefill Time vs. TOPS")
    plt.show()